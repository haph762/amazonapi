...
{
    public interface IMappingOrderStuller
    {
        public List<QgoldFtpOrderObject> ConvertOrderSave(List<OrderAmazon>? orderDB);
        public List<SubmitOrderParam> ConvertOrder(List<OrderAmazon>? orderDB);
    }
    public class MappingOrderStuller : IMappingOrderStuller
    {
        public List<QgoldFtpOrderObject> ConvertOrderSave(List<OrderAmazon>? orderDB)
        {
        ...
        private string GetShippingCode(string shipping_service, string country)
        {
            ...
            if (country.ToUpper() == "CA") return "UPS_STD_CANADA";

            var amz_shipping_service = shipping_service.ToUpper().Trim();

            Dictionary<string, string> dict = new Dictionary<string, string>();
            dict["Standard".ToUpper()] = "USPS_FIRST_CLASS";
            dict["SecondDay".ToUpper()] = "UPS_2DAY";
            dict["Expedited".ToUpper()] = "USPS_PRIORITY";
        ...
        public List<SubmitOrderParam> ConvertOrder(List<OrderAmazon>? orderDB)
        {
            List<SubmitOrderParam> orders = new List<SubmitOrderParam>();
            if (orderDB == null) return orders;
            foreach (var item in orderDB)
            {
                try
                {
                    List<ItemSubmit> item1 = new List<ItemSubmit>();
                    foreach (var i in item.OrderItemList)
                    {
                        if (i.SellerSKU.StartsWith("S-"))
                        {
                            item1.Add(new ItemSubmit()
                            {
                                Item = i.SellerSKU[2..],
                                Price = i.ItemPrice.Amount,
                                Qty = i.QuantityOrdered.ToString(),
                    ...
                    if (item1.Count > 0)
                    {
                        orders.Add(new SubmitOrderParam()
                        {
                            ShipToName = item.ShippingAddress.Name,
                            ShipToAddress1 = item.ShippingAddress.AddressLine1,
                            ShipToAddress2 = item.ShippingAddress.AddressLine2,
                            ShipToAddress3 = item.ShippingAddress.AddressLine3,
                            ShipToCity = item.ShippingAddress.City,
                            ShipToState = GetStateCode(item.ShippingAddress.StateOrRegion, item.ShippingAddress.CountryCode),
                            ShipToZip = item.ShippingAddress.PostalCode,
                            ShipToCountryCode = item.ShippingAddress.CountryCode,
                            ShipToProvince = null,
                            ShipToPhone = item.ShippingAddress.Phone,
                            Item = item1,
                            Description = null,
                            ShipMethodCode = GetShippingCode(item.ShipmentServiceLevelCategory, item.ShippingAddress.CountryCode),
                            MarketplaceOrderId = item.AmazonOrderId,
                            Marketplace = "Amazon"
                        });
                    }
                }
